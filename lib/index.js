...
var XMLHttpRequest = global && global.XMLHttpRequest ? global.XMLHttpRequest : require('xmlhttprequest').XMLHttpRequest;
...
Visitor.prototype = {
  ...
  send: function (fn) {
    ...
    var iterator = function () {
      ...
      //Set Request headers
      Object.keys(self.options.headers || {}).map(function(key) {
        xhr.setRequestHeader(key, self.options.headers[key]);
      });
      ...
      xhr.onload = function () {
        if (this.status >= 200 || this.status < 300) {
          ...
          nextIteration("HTTP error "+this.status+ ": "+this.responseText);
        }
      };
      xhr.onerror = function () {
        nextIteration("XHR error");
      };
      xhr.send(getBody(params));
    }
